...
export async function fetchFilteredInvoices( // async
  query: string,
  currentPage: number,
) {
  const offset = (currentPage - 1) * ITEMS_PER_PAGE;

  try { // await를 사용하려면 함수가 async로 정의되어있어야하고 await로 정의되면 아래 코드는 해당 코드(await sql<InvoicesTable>)가 실행완료 되기 전까지 실행될 수 없다
...
export async function fetchInvoicesPages(query: string) {
  ...
  `;
    ...
    const totalPages = Math.ceil(Number(count.rows[0].count) / ITEMS_PER_PAGE); // ITEMS_PER_PAGE 는 6임
    return totalPages;
  } catch (error) {
    console.error('Database Error:', error);
...
export async function fetchInvoiceById(id: string) {
  try {
    ...
    console.log(invoice); // Invoice 가 비었다면 array는 [] 로 출력된다
    return invoice[0];
  } catch (error) {
    console.error('Database Error:', error);
